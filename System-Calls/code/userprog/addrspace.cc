...
AddrSpace::AddrSpace(const AddrSpace* other, PCB* newpcb) {
    ...
    if (numPages <= memoryManager->getNumFreePages()) {
        ...
        for(int i = 0; i < numPages; i++){
            ...
            // get the i-th logical page table entry of "other" process
            TranslationEntry entry = (other->pageTable)[i];

            pageTable[i].virtualPage = entry.virtualPage;
            pageTable[i].physicalPage = memoryManager->getPage();
            pageTable[i].valid = entry.valid;
            pageTable[i].use = entry.use;
        ...
        for (int i = 0; i < numPages; i++) {
            ...
            // get the physical addres (by byte) of the other process,
            int src_physAddr = (other->pageTable)[i].physicalPage * PageSize;
            int dest_physAddr = pageTable[i].physicalPage * PageSize;

            // use bzero() to clean up and  use , bcopy() to complete the copy.
            bzero(&(machine->mainMemory[dest_physAddr]), PageSize);
            bcopy(&(machine->mainMemory[src_physAddr]), &(machine->mainMemory[dest_physAddr]), PageSize);
        }
