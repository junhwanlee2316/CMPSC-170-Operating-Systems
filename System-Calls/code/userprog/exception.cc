...
void yieldImpl() {
    ...
    //Yield using currentThread->Yield() to accomplish the context switch
    currentThread->Yield();
...
void exitImpl() {
    ...
    currentThread->space->getPCB()->status = P_GOOD;
...
void writeImpl() {
    ...
    if (fileID == ConsoleOutput) {
        ...
        int numActualBytesWrite = sysFile->file->WriteAt(buffer, size, userFile->currOffsetInFile);
        userFile->currOffsetInFile += numActualBytesWrite;
